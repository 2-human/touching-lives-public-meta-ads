...
const SLUG = pageSlug();
function reviewer(){let n=store.get('touching_lives_reviewer');if(!n){n=(window.prompt(L.namePrompt,'')||'Anonymous').trim()||'Anonymous';store.set('touching_lives_reviewer',n);}return n;}
...
function localAdapter(){
  const KEY='touching_lives_review_comments'; let cb=null;
 ...
 * querySelectorAll because a campaign-wide anchor can match several instances. */
...
function spotlight(anchor){
  SELANCHOR=anchor;
  let a=null;
  /* pages may expose __rwReveal to flip a dropdown/tab so the commented variant
   * is on screen, then return that zone's element. */
...
(async function init(){
  ...
  /* variant-aware anchors hook: pages that rewrite a zone's data-comment-id
   * when its variant selection changes call __rwRefresh() so the highlight
   * pass re-matches. */
